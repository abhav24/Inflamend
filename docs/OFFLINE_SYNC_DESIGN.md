...
- `AppState` schedules automatic retries for blocked or retryable mutations once their `nextRetryAt` time is due and reachability is online. It cancels scheduled retry work while offline/unknown and only retries eligible records.
- `NetworkReachabilityMonitor` uses `NWPathMonitor` for runtime online/offline state, with debug launch overrides for deterministic tests. Manual retry pauses while offline and leaves queued mutations pending.
...
- Profile exposes pending sync count, opens a detail sheet with per-record future action/target/attempt/next-retry/error metadata, automatic retry state, and network state, and lets users retry. Retry pauses while offline and currently marks queued records as blocked when online because Supabase is not configured.
...
Next app-layer work is to connect the replay worker and automatic retry scheduler to a real Supabase client, store returned server IDs and receipts, then implement conflict handling, auth refresh, and richer per-record remediation actions once credentials are available.
