...
- Local health-log edits coalesce into unreplayed creates when possible, existing-record edits reuse one pending update mutation, Home timeline edits preserve existing typed payloads for display-only changes, food timeline edits can replace meal/tag payloads, generic model edits can clear stale payloads, edit payload snapshots update with the latest local row, and edit-then-delete removes redundant pending updates.
