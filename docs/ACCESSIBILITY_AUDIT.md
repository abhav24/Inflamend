...
| Insights | Partial | Unverified | Mixed chart colors | Mostly OK; empty and populated summary paths, native range selector, and chart containers are identifier-backed and smoke-tested | Empty and populated summaries use explanatory text; range summary states Recent/All context; populated trend, bowel, and heatmap charts expose summary labels | In progress |
...
| Profile | Partial | Unverified | Likely acceptable | Rows OK; edit sheet, sync status, medication reminders, preferences, flare history, Care Plan, IBD Library, report export, data export, privacy toggles, sign-out, and destructive rows verified by UI smoke tests | Profile edit controls, sync blocked state, reminder setup state, preferences setup state, flare history empty/count states, Care Plan safety state, IBD Library source/safety state, and destructive actions use visible text | In progress |
...
- Added a native segmented Insights range selector, visible range summary text, stable identifiers, and UI smoke coverage for switching Recent to All.
...
- Added stable identifiers and UI smoke coverage for the Profile Edit button, editable display-name field, diagnosis/goal options, baseline count controls, flare-plan toggle, save action, and updated Profile header labels.
