...
- Add manual VoiceOver notes for privacy toggles, the Profile edit sheet, the Profile sync detail sheet including network and automatic retry status, the medication reminder settings sheet, the Profile Preferences sheet, the Profile Flare history sheet, the Profile Care Plan sheet, the Profile IBD Library sheet, and the voice permission fallback state.
