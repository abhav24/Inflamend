...
- Add per-record server IDs, conflict handling, and automatic retry execution.
...
- Add server IDs, conflict handling, automatic retry execution, and per-record failure details.
...
What remains:
- Add App Store screenshot assets and final icon review.
...
- Add server IDs, update/delete receipts, automatic retry execution, and conflict handling.
...
- Add conflict resolution, automatic retry execution, and richer user-visible per-record remediation details.
...
- Add structured edit controls for check-in fields.
- Add backend update replay and conflict handling for payload-bearing structured edits.
- Add user-managed medication schedule creation/editing, skipped/missed-dose history views, reminders, and manual Dynamic Type/VoiceOver verification for medication edit controls.
...
- Add live replay execution, conflict handling, richer per-record remediation actions, and manual Dynamic Type/VoiceOver verification for the sync detail sheet.
...
What remains:
- Connect the scheduled retry metadata and automatic retry scheduler to a live Supabase replay client.
- Add conflict handling, returned server IDs/receipts, and manual Dynamic Type/VoiceOver verification for the next-retry detail text.

## Current Checkpoint: Network Reachability Sync Pause

What changed:
- Added `SyncNetworkStatus` and a small `NetworkReachabilityMonitor` backed by `NWPathMonitor` so `AppState` can reflect unknown, online, and offline runtime network states.
- Added deterministic debug launch overrides for `--inflamend-network-offline` and `--inflamend-network-online` so UI tests can cover reachability without relying on simulator network state.
- Updated sync summaries and the Profile sync detail sheet to show offline reachability and explain that sync replay waits for connectivity.
- Changed manual sync retry so offline state pauses safely without changing queued records to backend-blocked, incrementing attempts, setting `nextRetryAt`, or writing backend errors.
- Added unit and UI smoke coverage for the offline sync pause path.
...
What passed:
- Focused offline reachability unit coverage passed, including queue preservation and offline sync summary assertions.
- Focused Profile offline sync UI smoke coverage passed, including network detail text, offline retry, and pending-row preservation.
- Full unit target passed with 45 tests.
- Clean build passed.

What failed during the loop:
- The first focused unit validation failed to compile because `AppState.deinit` touched the main-actor-isolated `toastTask` from a nonisolated context. The deinit cleanup was removed and the focused/full runs passed.

What remains:
- Connect the local retry scheduler to live Supabase replay once credentials exist.
- Add conflict handling, returned server IDs/receipts, Supabase Auth refresh handling, and manual Dynamic Type/VoiceOver verification for the network detail block.

## Current Checkpoint: Automatic Sync Retry Scheduling

What changed:
- Added due-retry eligibility on `PendingSyncMutation` so blocked or retryable records are retried only after their persisted `nextRetryAt` backoff time.
- Added an automatic sync retry task in `AppState` that schedules the next due retry while online, cancels while offline/unknown, and retries only eligible records instead of mutating the entire queue.
- Added a manual `runAutomaticSyncRetryIfEligible(at:)` seam for deterministic tests and future background/service integration.
- Added Profile sync detail copy for automatic retry state, including offline wait, reachability wait, next scheduled retry, and due-now states.
- Extended unit and UI smoke coverage for due automatic retry behavior and the Profile automatic-retry detail text.
...
What passed:
- Focused automatic retry unit coverage passed, including due-only retry, offline pause, attempt increment, next-backoff scheduling, and non-due queue preservation.
- Focused Profile sync UI smoke coverage passed for backend-blocked and offline sync states with the new automatic retry detail assertion.
- Full unit target passed with 46 tests.
- Clean build passed.

What failed during the loop:
- No implementation failures in this checkpoint. The scheduler compiled and passed focused/full validation on the first run.

What remains:
- Connect automatic retry execution to live Supabase replay once credentials exist.
- Store returned server IDs and receipts, add conflict handling, add Supabase Auth refresh handling, and manually verify the Profile sync detail sheet under Dynamic Type and VoiceOver.

## Current Checkpoint: Persisted Medication Dose Status

What changed:
- Added codable `MedicationDoseStatus` values for pending, taken, skipped, and missed medication doses.
- Added `medicationDoses` to the local snapshot so today's medication schedule/status survives app relaunch and remains exportable in the protected local snapshot.
- Switched the Log Meds schedule from local view `@State` to `AppState.medicationDoses`.
- Added `recordMedicationDoseStatus(id:status:)` so dose rows can update persisted status, refresh Home adherence, and queue a structured medication log with the selected status.
- Kept existing rapid medication logging and timeline medication edits compatible with manual adherence tests while synchronizing status back to the persisted dose list when the schedule and Home summary are aligned.
...
What passed:
- Focused medication dose persistence unit coverage passed, including skipped status persistence, Home adherence preservation, queued structured medication replay payloads, and snapshot restore.
- Focused medication UI smoke coverage passed for dose-taken Home summary updates and timeline status correction.
- Full unit target passed with 47 tests.
- Clean build passed.
...
What remains:
- Add user-managed medication schedule creation/editing, skipped/missed-dose history, real iOS notification scheduling, backend medication sync, and manual Dynamic Type/VoiceOver verification for the richer status row.

## Current Checkpoint: Medication Reminder Settings

What changed:
- Added `MedicationReminderSettings` to the local snapshot with legacy decode fallback, allowed lead times, setup-required status, and exportable fields.
- Added `AppState` reminder preference methods that persist enable/disable and lead-time changes, normalize unsupported values, and queue a future `privacyPreference` replay mutation for `public.user_settings`.
- Replaced the Profile "Medication reminders" placeholder toast with a settings sheet that exposes a native toggle, lead-time segmented buttons, scheduled dose preview, and explicit notification-setup-required copy.
- Added Supabase migration scaffold `0004_medication_reminder_settings.sql` for `user_settings.medication_reminder_lead_time_minutes`.
- Kept the scope honest: this checkpoint saves reminder intent locally but does not schedule iOS notifications because notification entitlement/authorization setup is still externally pending.
...
What passed:
- Focused reminder unit coverage passed for local persistence, snapshot restore, JSON export decode, setup-required state, and queued user-settings replay mutation.
- Focused Profile UI smoke coverage passed for opening reminder settings, enabling the local preference, selecting a 30-minute lead time, and verifying notification setup copy.
- Full unit target passed with 48 tests.
- Clean build passed with only pre-existing warnings in unrelated files.

What failed during the loop:
- The first focused unit assertion checked pretty-printed JSON with an exact no-space substring. I replaced it with structured decode assertions and the focused test passed.
- Supabase CLI migration reset/verification was not run because the Supabase CLI is not installed and project credentials remain externally unavailable in this run.

What remains:
- Add real notification authorization, local notification scheduling/cancellation, medication schedule editing, backend settings sync, and manual Dynamic Type/VoiceOver verification for the reminder sheet.

## Current Checkpoint: Profile Preferences and Weight Units

What changed:
- Added `AppPreferences` and `WeightUnit` to the local snapshot with legacy decode fallback, exportable fields, and device-timezone normalization.
- Replaced the Profile "Preferences" placeholder with a settings sheet for preferred weight unit and device timezone behavior.
- Updated Weight logging so new entries use the selected unit instead of hard-coded kilograms.
- Added unit correction controls to the Home timeline weight edit sheet so existing weight entries can be corrected between KG and LB.
- Added Supabase migration scaffold `0005_app_preferences.sql` for preferred weight unit and timezone preference columns on `public.user_settings`.
...
What passed:
- Focused preferences unit coverage passed for local persistence, snapshot restore, JSON export decode, queued settings replay, and weight logs saved with the preferred unit.
- Focused Profile preferences UI smoke coverage passed for changing the preferred weight unit to LB and saving a Weight log that appears as `62.4 lb`.
- Existing Weight timeline edit UI smoke coverage still passed for the default KG flow.
- Full unit target passed with 49 tests.
- Clean build passed with only pre-existing warnings in unrelated files.
...
What remains:
- Add backend settings serialization, conflict handling, richer timezone editing, unit conversion helpers, and manual Dynamic Type/VoiceOver verification for the Preferences and Weight edit sheets.

## Current Checkpoint: Local Flare History

What changed:
- Added `FlareHistoryBuilder`, `FlareHistorySummary`, and `FlareHistoryEvent` to derive flare-marked local logs using the same typed payload-aware flare predicate as Insights.
- Added `AppState.flareHistorySummary` so Profile can show a live flare-mark count without duplicating parsing in the view.
- Replaced the Profile "Flare history" placeholder toast with a sheet that shows an empty state or recent flare-marked log rows.
- Added stable accessibility identifiers for the Profile row, flare-history sheet title/summary, empty state, and event rows.
...
What passed:
- Focused flare-history unit coverage passed for typed flare check-ins, text flare notes, newest-first ordering, row summaries, and non-flare exclusion.
- Focused Profile UI smoke coverage passed for creating a local flare mark, seeing the Profile row count, opening the sheet, and verifying the first event row.
- Full unit target passed with 50 tests.
- Clean build passed with only pre-existing warnings in unrelated files.
...
What remains:
- Add backend flare-history sync/query support, richer episode grouping, date filtering, clinician-report integration, and manual Dynamic Type/VoiceOver verification for the sheet.

## Current Checkpoint: Local Care Plan Questions

What changed:
- Added `CarePlanBuilder`, `CarePlanSummary`, and `CarePlanQuestion` to generate local GI-visit preparation questions from onboarding profile, flare history, and safety context.
- Added `AppState.carePlanSummary` so Profile can show a live local question count without duplicating logic in the view.
- Replaced the Profile "Care plan" placeholder toast with a sheet that lists preparation questions and visible safety framing.
- Added stable accessibility identifiers for the Profile row, Care Plan sheet title/summary, question rows, and safety note.
...
What passed:
- Focused Care Plan unit coverage passed for diagnosis, missing flare plan, local flare marks, safety-message insertion, and avoiding medication-change treatment advice.
- Focused Profile UI smoke coverage passed for opening the Care Plan sheet and verifying local questions plus the non-treatment-plan safety note.
- Full unit target passed with 51 tests.
- Clean build passed with only pre-existing warnings in unrelated files.
...
What remains:
- Add editable/exportable care-plan notes, backend persistence, clinician-report integration, richer appointment-prep workflows, and manual Dynamic Type/VoiceOver verification for the sheet.

## Current Checkpoint: Local IBD Education Library

What changed:
- Added `PatientEducationLibraryBuilder`, `PatientEducationLibrary`, `PatientEducationArticle`, and `PatientEducationSource` to provide a small local, source-attributed IBD education library.
- Added `AppState.ibdEducationLibrary` so Profile can show a live local guide count without hardcoding article state in the view.
- Replaced the Profile "IBD library" placeholder toast with a sheet that lists source-framed guides, clinician discussion prompts, and visible non-treatment safety framing.
- Added stable accessibility identifiers for the Profile row, library sheet title/summary, article rows, article source labels, and safety note.
- Verified the education copy against current CDC, NIDDK, and Crohn's & Colitis Foundation public information and kept the in-app copy limited to education and appointment preparation.
...
What passed:
- Focused education-library unit coverage passed for local guide count, source metadata, HTTPS source URLs, CDC/NIDDK/Crohn's & Colitis Foundation attribution, and non-prescriptive safety framing.
- Focused Profile UI smoke coverage passed for opening the IBD Library sheet and verifying guide count, IBD basics content, source attribution, nutrition caution, and the non-diagnostic safety note.
- Full unit target passed with 52 tests.
- Clean build passed with only pre-existing warnings in unrelated files.

What failed during the loop:
- The first attempt to run focused unit and UI checks in parallel failed one `xcodebuild` process with a shared DerivedData build database lock. The surviving focused unit run passed, and the focused UI smoke test passed when rerun sequentially.

What remains:
- Add editable/bookmarkable education notes, richer article search/filtering, external-source link handling, backend content/version sync, clinician-report integration, and manual Dynamic Type/VoiceOver verification for the sheet.

## Current Checkpoint: Local Profile Editing

What changed:
- Added `AppState.updateProfile` to update the local display name and onboarding profile fields while preserving the local user ID and original profile completion date.
- The update path trims display names, clamps baseline stool count to `0...20`, marks "Prefer not to say" as sensitive-question skipped state, persists to the protected local snapshot, and queues future auth/onboarding sync mutations when values change.
- Replaced the Profile header "Edit" placeholder toast with a sheet for display name, diagnosis, primary goal, baseline stool count, and clinician flare-plan status.
- Added stable accessibility identifiers for the Profile edit button, visible display name/diagnosis labels, edit sheet title, name field, option buttons, baseline count controls, flare-plan toggle, safety note, keyboard Done action, and save button.
...
What passed:
- Focused Profile edit unit coverage passed for local persistence, display-name trimming, baseline stool count clamping, local user ID preservation, restored snapshot values, and queued future auth/onboarding sync mutations.
- Focused Profile UI smoke coverage passed for opening the editor, editing the display name, changing diagnosis and primary goal, incrementing baseline count, toggling flare-plan status, saving, and verifying the Profile header updates.
- Full unit target passed with 53 tests.
- Clean build passed with only pre-existing warnings in unrelated files.
...
What remains:
- Add Supabase profile/settings update replay, server conflict handling, profile edit receipts, richer validation/error copy, and manual Dynamic Type/VoiceOver verification for the edit sheet.

## Current Checkpoint: App Icon Artwork

What changed:
- Added deterministic source artwork generation in `scripts/generate_app_icon.py`.
- Generated `Inflamend/Assets.xcassets/AppIcon.appiconset/AppIcon-1024.png` as an opaque 1024x1024 RGB PNG using the app sage/ink/clay palette.
- Updated `Inflamend/Assets.xcassets/AppIcon.appiconset/Contents.json` to reference the generated universal iOS app icon file.
- Performed a local visual inspection of the generated icon for framing, opacity, and basic quality.

What was tested:
- `python3 scripts/generate_app_icon.py`
- `python3 -m json.tool Inflamend/Assets.xcassets/AppIcon.appiconset/Contents.json`
- `python3 - <<'PY' ... Image.open(...).mode/size ... PY`
- `file Inflamend/Assets.xcassets/AppIcon.appiconset/AppIcon-1024.png`
- `xcodebuild clean build -quiet -scheme Inflamend -destination 'platform=iOS Simulator,name=iPhone 17'`

What passed:
- Generator produced the app icon PNG in the asset catalog.
- Asset catalog JSON parsed successfully.
- PNG validation reported RGB mode, 1024x1024 size, and no alpha channel.
- `file` reported 1024x1024 8-bit RGB PNG data.
- Clean build passed with only pre-existing warnings in unrelated files, confirming Xcode accepts the catalog.

What failed during the loop:
- `plutil -lint Inflamend/Assets.xcassets/AppIcon.appiconset/Contents.json` rejected the JSON opening brace on this machine, so JSON validation was performed with Python instead.

What remains:
- Add App Store screenshot assets, launch-screen visual QA, final brand review, and release-archive resource checks before submission.

## Current Checkpoint: Privacy Policy and Terms Drafts

What changed:
- Added `docs/PRIVACY_POLICY_DRAFT.md` with current local-storage behavior, planned Supabase/AI service-provider paths, user controls, deletion/export limits, retention gaps, child-safety status, security expectations, and medical-safety disclaimers.
- Added `docs/TERMS_OF_USE_DRAFT.md` with usage boundaries, medical/emergency disclaimers, user responsibilities, account/backend status, user-content handling, Care/education limitations, export cautions, payment status, and legal sections requiring counsel review.
- Updated `docs/APP_STORE_READINESS.md` and `docs/APP_STORE_METADATA.md` so App Store legal-copy work points to concrete draft source files instead of blank placeholders.

What was tested:
- Repository text search for legal/App Store placeholder language and current metadata references.

What passed:
- Draft documents were added without secrets, PHI, published legal claims, HIPAA claims, or fake URLs.
- App Store readiness now distinguishes drafted legal text from the still-blocked public URL/legal-review requirement.

What failed during the loop:
- No implementation failures in this checkpoint.

What remains:
- Legal counsel review, company/contact details, public privacy policy URL, public terms URL, data deletion URL/workflow, jurisdiction-specific legal sections, and final App Store privacy questionnaire answers.

## Current Checkpoint: Insights Range Selector

What changed:
- Replaced the string-backed Insights range state with a typed `InsightRangeOption`.
- Moved the Recent/All control into a native segmented picker so it remains visible and tappable on compact iPhone widths.
- Added a visible range summary under the confidence text, with `Last 7 days` and `All local logs` copy tied to the selected range.
- Added stable accessibility identifiers for the range selector and visible range summary.
- Added `testInsightsRangeToggleUpdatesSummarySmoke`, which creates a local flare log, opens Insights, verifies Recent summary copy, switches to All, and verifies the summary updates.
...
What passed:
- Focused Insights range UI smoke coverage passed after explicitly booting the iPhone 17 simulator.
- Clean build passed with only pre-existing warnings in unrelated files.

What failed during the loop:
- The first focused UI test run failed before app launch with Xcode simulator runner error `Mach error -308`; explicitly booting the simulator resolved the launcher issue.
- Earlier custom button attempts exposed a compact-width layout/accessibility problem where the selector element had no usable visible frame; the final implementation uses a native segmented picker.

What remains:
- Add custom report/export date-range controls, backend-synced summary parity, richer partial-data range scenarios, and manual VoiceOver/Dynamic Type review for the Insights header and selector.

## Current Checkpoint: Auth Invalid Email Feedback

What changed:
- Added persistent inline auth validation feedback for invalid email input in `AuthGateView`.
- Added a stable `auth-error-message` accessibility identifier so validation copy is testable and available outside the transient toast.
- Routed the auth gate through the shared `AppState.isValidEmail` helper before local sign-up/sign-in scaffold calls.
- Added `testInvalidAuthEmailShowsInlineErrorSmoke`, which submits an invalid local auth email, verifies the inline error, and confirms the app does not advance to onboarding.
...
What passed:
- Focused invalid-auth UI smoke coverage passed.
- Clean build passed with only pre-existing warnings in unrelated files.

What failed during the loop:
- The first focused UI attempt asserted the transient toast label, but the toast text was not exposed as a stable `staticTexts` element in XCTest. The test was tightened to the durable product contract: inline validation appears and onboarding does not start.

What remains:
- Replace the local auth scaffold with Supabase Auth, Keychain-backed sessions, server-backed auth error handling, password validation policy, and manual VoiceOver/Dynamic Type review for auth validation copy.
...
| Pass 2 | Core product/backend/UX implementation | In progress | Supabase schema/RLS/functions scaffolded; auth sign-up/sign-in/onboarding and invalid-email feedback UI smoke coverage, local profile editing, session restore/local persistence, structured log timestamps with legacy snapshot decode, typed local log payloads with legacy fallback, timeline edit typed-payload preservation, persisted medication dose status, local medication reminder settings, persisted Profile preferences and weight units, local Profile flare history, local Care Plan questions, local source-attributed IBD education library, food, bowel, symptom, sleep, weight, medication, and check-in timeline structured editing, date-windowed Insights/report summaries with explicit Recent/All selector UI smoke coverage, local pending sync queue with replay planning, health-log payload snapshots, idempotency metadata, retry backoff metadata, automatic scheduled retry execution, reachability-aware offline retry pause, per-record blocked error metadata, and Profile blocked/offline retry detail UI smoke coverage, core logging with Today check-in/correction, bowel red-flag, food, symptom, medication dose tracking/editing, editable voice confirmation, native voice permission fallback, and timeline edit/delete/undo UI smoke coverage, data-backed Insights with empty-state, populated local-log, range-selector, and chart accessibility-summary UI smoke coverage, local doctor-report export with UI smoke coverage, local user-data export, local Care safety responses with red-flag and medication-refusal UI smoke coverage, destructive-action confirmations, privacy controls with toggle UI smoke coverage, Profile edit/sign-out/export/destructive/reminder/preferences/flare-history/care-plan/IBD-library UI smoke tests wired, preview asset catalog packaging cleaned up, deterministic app icon artwork generated, and privacy policy/terms drafts added |
...
|---|---|---|---|
| Welcome/auth | Scaffolded locally with UI smoke coverage | `AuthGateView`, inline invalid-email feedback, `AuthSession`, `testFreshSignUpCompletesOnboardingSmoke`, `testInvalidAuthEmailShowsInlineErrorSmoke` | Replace with Supabase Auth and Keychain token storage |
| Sign up | Scaffolded locally with UI smoke coverage | Local account creation validates email, invalid email stays on Auth with inline error, and fresh UI smoke test reaches onboarding | Replace password scaffold with Supabase Auth |
| Sign in | Scaffolded locally with UI smoke coverage | Local sign-in validates email through the shared auth gate guard, creates a local session, and reaches onboarding; `testLocalSignInReachesOnboardingSmoke` | Replace with Supabase Auth/errors |
...
| Onboarding | Scaffolded locally with edit UI smoke coverage | `OnboardingGateView`, `OnboardingProfile`, default completion UI smoke test; Profile edit sheet can update local diagnosis, goal, baseline stool count, and flare-plan status; `testProfileEditPersistsSessionProfileAndQueuesSync`; `testProfileEditUpdatesLocalHeaderSmoke` | Sync profile updates to backend and add conflict handling/receipts |
...
| Flare history | Implemented locally with UI smoke coverage | Profile Flare history row and sheet derive flare-marked events from typed check-ins and local log text through `FlareHistoryBuilder`; `testFlareHistoryBuilderUsesTypedFlareLogs`; `testProfileFlareHistoryShowsLocalFlareMarksSmoke` | Add backend flare-history query/sync support, episode grouping, date filtering, report integration, and manual accessibility verification |
...
| Medication tracking | Implemented with persisted local dose status, local reminder preferences, structured timeline editing, and UI smoke coverage | `MedicationDoseStatus`, `MedicationReminderSettings`, snapshot-backed `medicationDoses`, quick meds and full-row Meds form dose buttons update dose count/logs with typed medication payloads; Profile reminder settings persist enable/lead-time intent and queue future user-settings replay; Home timeline editing can update medication dose status, replay snapshots, and adherence summary state; `testMedicationDoseStatusPersistsAndQueuesStructuredLog`; `testMedicationReminderSettingsPersistExportAndQueuePreference`; `testMedicationDoseUpdatesHomeSummarySmoke`; `testMedicationLogCanBeEditedFromTimelineSmoke`; `testProfileMedicationReminderSettingsSmoke`; `testUpdateLogCanReplaceMedicationPayloadAndReconcileAdherence` | Add user-managed schedule creation/editing, real iOS notification scheduling, backend sync, skipped/missed-dose history views, and manual accessibility verification for medication controls |
...
| Weight logging | Implemented with local persistence, preferred units, structured timeline editing, and UI smoke coverage | Weight form saves validated manual entries through `recordWeight` using `AppPreferences.weightUnit`; typed weight payloads persist value/unit; Home timeline editing can update typed weight values, unit, and replay snapshots; `testProfilePreferencesWeightUnitDrivesWeightLogSmoke` verifies Profile LB preference drives a new Weight log; `testWeightLogCanBeEditedFromTimelineSmoke` saves a default kg log and edits 62.4 kg to 63.0 kg; `testAppPreferencesPersistExportAndWeightLoggingUsesPreferredUnit`; `testUpdateLogCanReplaceWeightPayloadAndReplaySnapshot` covers payload replacement and replay snapshots | Add backend settings/log sync, unit conversion helpers, and manual accessibility verification for the edit controls |
...
| Offline sync queue | Implemented locally with replay planning, retry backoff metadata, automatic scheduled retry execution, reachability-aware pause, per-record blocked details, and UI smoke coverage | `PendingSyncMutation`, `SyncReplayPlanItem`, `LocalSyncReplayWorker`, `SyncMutationPayload`, `HealthLogReplayPayload`, `SyncNetworkStatus`, `NetworkReachabilityMonitor`, health-log payload snapshots for create/update replay, idempotency/server/receipt metadata, `nextRetryAt`, due-retry scheduler, `healthLogUpdate`, `healthLogDeletion`, Profile sync row and detail sheet, queue persistence/edit/delete/replay-plan/backoff/reachability/automatic-retry/legacy-mutation tests, `testAutomaticSyncRetryRunsOnlyDueMutationsWhenOnline`, `testOfflineReachabilityPausesSyncRetryWithoutMutatingQueue`, `testSyncReplayBackoffSchedulesAndResetsAfterLocalEdit`, `testSyncReplayPlanCarriesHealthLogPayloadSnapshots`, `testProfileSyncRetryShowsBackendBlockedSmoke`, `testProfileSyncRetryPausesWhenNetworkOfflineSmoke` | Connect Supabase replay client, returned server IDs/receipts, live payload serialization, conflicts, auth refresh, and richer per-record remediation actions |
...
| Insights | Implemented locally with empty, populated, typed-payload, date-windowed, range-selector, and chart-summary coverage | Local logs with `loggedAt` timestamps and typed payloads drive 7-day Recent and All summaries, bowel chart, pain heatmap, food frequency rows, empty states, a visible Recent/All segmented selector summary, and populated chart accessibility labels; Home timeline edits preserve typed payloads used by summaries; `testInsightSummaryPrefersTypedPayloadOverDisplayText`; `testInsightSummaryRecentRangeUsesLoggedAtDates`; `testUpdateLogCanPreserveTypedPayloadsForTimelineEdits`; `testInsightsEmptyStateAvoidsDemoClaimsSmoke`; `testInsightsPopulatedSummaryUsesLocalLogsSmoke`; `testInsightsRangeToggleUpdatesSummarySmoke` | Add custom report/export range UI, backend-synced summaries, richer populated/partial scenarios, export/share integration, type-specific structured edit controls, and manual VoiceOver/Dynamic Type verification |
...
| Profile/settings | Partial with local edit/reminder/preferences/flare-history/education UI smoke coverage | Profile reflects restored session/profile and local stats; Profile edit sheet updates display name plus onboarding profile fields and queues future auth/onboarding sync; Medication reminder settings sheet persists a local enable/lead-time preference and states that notification setup is still required; Preferences sheet persists preferred weight unit and device timezone behavior; Flare history sheet shows local flare-marked logs; IBD Library sheet shows local source-attributed education guides; `testProfileEditPersistsSessionProfileAndQueuesSync`; `testProfileEditUpdatesLocalHeaderSmoke`; `testProfileMedicationReminderSettingsSmoke`; `testProfilePreferencesWeightUnitDrivesWeightLogSmoke`; `testProfileFlareHistoryShowsLocalFlareMarksSmoke`; `testProfileIBDLibraryShowsSourceFramedGuidesSmoke` | Connect real notification authorization/scheduling, richer timezone editing, backend profile/settings sync/replay, backend flare-history queries, and backend education content/version sync |
| Care plan | Implemented locally with UI smoke coverage | Profile Care Plan sheet uses onboarding profile, flare history, and safety state to generate visit-preparation questions with non-treatment safety framing; `CarePlanBuilder`; `testCarePlanBuilderCreatesPreparationQuestionsWithoutTreatmentAdvice`; `testProfileCarePlanShowsLocalQuestionsSmoke` | Add editable/exportable notes, backend persistence, clinician-report integration, richer appointment workflows, and manual accessibility verification |
| IBD education library | Implemented locally with UI smoke coverage | Profile IBD Library sheet provides four source-attributed education guides with clinician prompts and non-treatment safety framing; sources include CDC, NIDDK, and Crohn's & Colitis Foundation public pages; `PatientEducationLibraryBuilder`; `testPatientEducationLibraryUsesSourceFramedNonTreatmentCopy`; `testProfileIBDLibraryShowsSourceFramedGuidesSmoke` | Add editable/bookmarkable notes, source link handling, article search/filtering, backend content/version sync, clinician-report integration, and manual accessibility verification |
...
- Tests: passing with 53 unit tests through `InflamendTests` and 36 UI smoke tests through `InflamendUITests`.
- Improvement passes completed: pass 1 is complete; pass 2 is in progress.
- Core flows: auth sign-up/sign-in/onboarding and invalid-email feedback with UI smoke coverage, local Profile editing with UI smoke coverage, Profile sign-out with UI smoke coverage, Profile sync status with replay planning/idempotency/backoff/reachability/automatic-retry metadata, health-log replay payload snapshots, blocked-retry and offline-pause UI smoke coverage, and per-record blocked detail sheet, Profile privacy toggles, medication reminder settings, app preferences, flare history, Care Plan questions, and IBD Library education guides with UI smoke coverage, Today check-in with save/edit UI smoke coverage, structured log timestamps with legacy decode coverage, typed local log payloads with legacy fallback, Home timeline edit typed-payload preservation, persisted medication dose status, reminder preferences, and app preferences, local flare history, local Care Plan questions, local IBD education library, food/bowel/symptom/sleep/weight/medication/check-in timeline structured editing, date-windowed Insights/report summaries with explicit range selector coverage, timeline log edit/delete/undo with UI smoke coverage, bowel red-flag logging with UI smoke coverage, food logging with UI smoke coverage, symptom logging/editing with UI smoke coverage, sleep logging/editing with UI smoke coverage, weight logging/editing/unit preference with UI smoke coverage, medication dose tracking/editing with UI smoke coverage, voice transcript confirmation with editable-field UI smoke coverage, voice permission fallback with UI smoke coverage, session restore, local persistence, pending sync queue, logging, local-log Insights with empty, populated, typed-payload, range-selector, and chart accessibility-summary UI smoke coverage, local text doctor-report export with Profile UI smoke coverage, local user-data JSON export with Profile UI smoke coverage, local Care safety responses with red-flag and medication-refusal UI smoke coverage, destructive-action confirmations with UI smoke coverage, safety, privacy, and report scaffolds improved; live Supabase auth/sync/backend integration remains incomplete.
...
- Safety/privacy/App Store readiness: foundational docs, privacy manifest, Swift red-flag logic, generated microphone/speech usage strings, visible safety/privacy UI, auth sign-up/sign-in/onboarding and invalid-email feedback UI smoke coverage, Profile edit UI smoke coverage, Profile sign-out UI smoke coverage, Profile sync blocked-state/detail UI smoke coverage, Profile privacy-toggle, medication-reminder, preferences, flare-history, Care Plan, and IBD Library UI smoke coverage, Today check-in save/edit UI smoke coverage, timeline edit/delete/undo UI smoke coverage, Insights empty-state, populated local-log, typed-payload summary, date-windowed summary, range-selector summary, and chart-summary UI smoke coverage, Log bowel red-flag UI smoke coverage, Log food UI smoke coverage, Log symptom UI smoke coverage, Log sleep UI smoke coverage, Log weight UI/unit UI smoke coverage, Log medication dose/edit UI smoke coverage, Log voice confirmation UI smoke coverage, Log voice permission fallback UI smoke coverage, Care red-flag UI smoke coverage, Care medication-refusal UI smoke coverage, Profile doctor-report export UI smoke coverage, Profile user-data export UI smoke coverage, destructive confirmation UI smoke coverage, typed local sync payload privacy documentation, local source-attributed education safety framing, preview asset resource cleanup, generated app icon artwork, and privacy policy/terms drafts now exist; broader UI tests, legal review, public URLs, screenshots, and final release checks remain.
- Git checkpoints: baseline, backend scaffold, health logic tests, core UX, auth/persistence, sync queue, Insights, report export, Care safety, privacy confirmation, user-data export, UI smoke target, destructive confirmation UI, auth/onboarding UI, Care red-flag UI, Today check-in UI, Profile sign-out UI, local sign-in UI, bowel red-flag UI, Care medication-refusal UI, Insights empty-state UI, doctor-report export UI, medication dose UI, food log UI, voice confirmation UI, privacy toggle UI, voice permission fallback, Profile sync status UI, asset catalog cleanup, populated Insights UI, timeline log delete, Insights chart accessibility, timeline log edit, sync replay planner, timeline delete undo, sync replay metadata, structured log timestamps, date-windowed Insights/report, typed local log payload, sync replay payload snapshot, timeline edit payload preservation, food timeline structured edit, bowel timeline structured edit, symptom timeline structured edit, sleep timeline structured edit, weight timeline structured edit, medication timeline structured edit, check-in timeline structured edit, Profile sync detail, sync retry backoff, network reachability sync pause, automatic sync retry scheduling, persisted medication dose status, medication reminder settings, Profile preferences/weight units, local flare history, local Care Plan questions, local IBD education library, local Profile editing, app icon artwork, privacy policy/terms drafts, Insights range selector, and auth invalid-email feedback checkpoints exist.
