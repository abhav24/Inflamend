...
What remains:
- Add recent/favorite foods, edit support, backend sync, and populated Insights food-pattern UI coverage.
...
- Add edit support for local logs and backend deletion replay.
...
What remains:
- Add edit support, undo affordances, and richer per-record history.
...
| Pass 2 | Core product/backend/UX implementation | In progress | Supabase schema/RLS/functions scaffolded; auth sign-up/sign-in/onboarding UI smoke coverage, session restore/local persistence, local pending sync queue with Profile blocked-retry UI smoke coverage, core logging with Today check-in, bowel red-flag, food, medication dose, editable voice confirmation, native voice permission fallback, and timeline delete confirmation UI smoke coverage, data-backed Insights with empty-state and populated local-log UI smoke coverage, local doctor-report export with UI smoke coverage, local user-data export, local Care safety responses with red-flag and medication-refusal UI smoke coverage, destructive-action confirmations, privacy controls with toggle UI smoke coverage, Profile sign-out/export/destructive UI smoke tests wired, and preview asset catalog packaging cleaned up |
...
| Today check-in | Implemented with local persistence and UI smoke coverage | `CheckInSheet`, `AppState.recordCheckIn`, snapshot store, `testTodayCheckInSavesToTimelineSmoke`; timeline deletion can remove created entries after confirmation | Add backend sync and edit support |
| Bowel movement logging | Implemented with local persistence and UI smoke coverage | Quick Bristol and detailed bowel form call `recordBowel`; `testBowelLogWithSignificantBloodShowsSafetyGuidanceSmoke`; timeline deletion can remove created entries after confirmation | Add backend sync, edit support, and stricter validation |
| Food/meal logging | Implemented with local persistence and UI smoke coverage | Quick food and Food form insert timeline logs; `testFoodLogSavesPatternEntrySmoke` verifies pattern-tracking wording; `testInsightsPopulatedSummaryUsesLocalLogsSmoke` verifies food-frequency appearance in populated Insights; timeline deletion can remove created entries after confirmation | Add backend sync, recent/favorite foods, edit support, and broader populated food-pattern Insights scenarios |
...
| Notes logging | Implemented with local persistence | `LogNoteForm`; timeline deletion can remove note entries after confirmation | Add edit support and backend sync |
| Timeline log deletion | Implemented locally with confirmation and UI smoke coverage | `AppState.deleteLog`, `healthLogDeletion`, `testDeleteLogRemovesEntryAndCoalescesPendingCreate`, `testTimelineEntryDeleteRequiresConfirmationSmoke` | Add edit support, undo, backend deletion replay, conflict handling, and deletion receipts |
| Offline sync queue | Implemented locally with UI smoke coverage | `PendingSyncMutation`, `healthLogDeletion`, Profile sync row, queue persistence/deletion tests, `testProfileSyncRetryShowsBackendBlockedSmoke` | Add Supabase replay worker, server IDs, conflicts, backoff, reachability, and per-record failure detail |
...
| Insights | Implemented locally with empty and populated UI smoke coverage | Local logs drive trend summaries, bowel chart, pain heatmap, food frequency rows, and empty states; `testInsightsEmptyStateAvoidsDemoClaimsSmoke`; `testInsightsPopulatedSummaryUsesLocalLogsSmoke` | Add structured dated records, chart accessibility summaries, backend-synced summaries, richer populated scenarios, and export/share integration |
...
| Doctor report/export | Implemented locally with UI smoke coverage | Profile report row writes a protected local text file and presents `ShareLink`; `testProfileDoctorReportExportSheetSmoke` | Add CSV/PDF output, structured ranges, backend export jobs, richer share/export verification, and report download receipts |
...
- Tests: passing with 25 unit tests through `InflamendTests` and 19 UI smoke tests through `InflamendUITests`.
- Improvement passes completed: pass 1 is complete; pass 2 is in progress.
- Core flows: auth sign-up/sign-in/onboarding with UI smoke coverage, Profile sign-out with UI smoke coverage, Profile sync status with blocked-retry UI smoke coverage, Profile privacy toggles with UI smoke coverage, Today check-in with UI smoke coverage, timeline log deletion with confirmation UI smoke coverage, bowel red-flag logging with UI smoke coverage, food logging with UI smoke coverage, medication dose tracking with UI smoke coverage, voice transcript confirmation with editable-field UI smoke coverage, voice permission fallback with UI smoke coverage, session restore, local persistence, pending sync queue, logging, local-log Insights with empty and populated UI smoke coverage, local text doctor-report export with Profile UI smoke coverage, local user-data JSON export with Profile UI smoke coverage, local Care safety responses with red-flag and medication-refusal UI smoke coverage, destructive-action confirmations with UI smoke coverage, safety, privacy, and report scaffolds improved; live Supabase auth/sync/backend integration remains incomplete.
