...
| Profile/settings | Partial with local reminder/preferences/flare-history/education UI smoke coverage | Profile reflects restored session/profile and local stats; Medication reminder settings sheet persists a local enable/lead-time preference and states that notification setup is still required; Preferences sheet persists preferred weight unit and device timezone behavior; Flare history sheet shows local flare-marked logs; IBD Library sheet shows local source-attributed education guides; `testProfileMedicationReminderSettingsSmoke`; `testProfilePreferencesWeightUnitDrivesWeightLogSmoke`; `testProfileFlareHistoryShowsLocalFlareMarksSmoke`; `testProfileIBDLibraryShowsSourceFramedGuidesSmoke` | Connect real notification authorization/scheduling, richer timezone editing, backend profile/settings sync, backend flare-history queries, and backend education content/version sync |
