...
| Today check-in | Implemented with local persistence and UI smoke coverage | `CheckInSheet`, `AppState.recordCheckIn`, snapshot store, `LogEntry.loggedAt`, typed check-in payloads, `testTodayCheckInSavesToTimelineSmoke`; Home timeline editing can update local title/details while preserving typed payloads, and deletion can remove entries after confirmation with a local undo toast | Add backend sync and structured type-specific edit fields |
| Bowel movement logging | Implemented with local persistence and UI smoke coverage | Quick Bristol and detailed bowel form call `recordBowel`; typed bowel payloads persist Bristol, urgency, blood, mucus, pain, and nighttime fields; `testBowelLogWithSignificantBloodShowsSafetyGuidanceSmoke`; Home timeline editing can update local title/details while preserving typed payloads, and deletion can remove entries after confirmation with a local undo toast | Add backend sync, type-specific edit fields, and stricter validation |
| Food/meal logging | Implemented with local persistence and UI smoke coverage | Quick food and Food form insert timeline logs with typed food payloads; `testFoodLogSavesPatternEntrySmoke` verifies pattern-tracking wording; `testInsightsPopulatedSummaryUsesLocalLogsSmoke` verifies food-frequency appearance in populated Insights; Home timeline editing can update local title/details while preserving food tags in the typed payload, and deletion can remove entries after confirmation with a local undo toast | Add backend sync, recent/favorite foods, structured edit fields, and broader populated food-pattern Insights scenarios |
