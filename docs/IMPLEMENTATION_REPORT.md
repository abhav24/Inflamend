...
| Pass 2 | Core product/backend/UX implementation | In progress | Supabase schema/RLS/functions scaffolded; auth sign-up/sign-in/onboarding UI smoke coverage, session restore/local persistence, structured log timestamps with legacy snapshot decode, typed local log payloads with legacy fallback, timeline edit typed-payload preservation, persisted medication dose status, local medication reminder settings, food, bowel, symptom, sleep, weight, medication, and check-in timeline structured editing, date-windowed Insights/report summaries, local pending sync queue with replay planning, health-log payload snapshots, idempotency metadata, retry backoff metadata, automatic scheduled retry execution, reachability-aware offline retry pause, per-record blocked error metadata, and Profile blocked/offline retry detail UI smoke coverage, core logging with Today check-in/correction, bowel red-flag, food, symptom, medication dose tracking/editing, editable voice confirmation, native voice permission fallback, and timeline edit/delete/undo UI smoke coverage, data-backed Insights with empty-state, populated local-log, and chart accessibility-summary UI smoke coverage, local doctor-report export with UI smoke coverage, local user-data export, local Care safety responses with red-flag and medication-refusal UI smoke coverage, destructive-action confirmations, privacy controls with toggle UI smoke coverage, Profile sign-out/export/destructive/reminder UI smoke tests wired, and preview asset catalog packaging cleaned up |
...
| Profile/settings | Partial with local reminder UI smoke coverage | Profile reflects restored session/profile and local stats; Medication reminder settings sheet persists a local enable/lead-time preference and states that notification setup is still required; `testProfileMedicationReminderSettingsSmoke` | Connect real notification authorization/scheduling and backend profile/settings sync |
