...
- Wire typed payloads to Supabase health-log columns and replay payload serialization once credentials exist.
...
| Today check-in | Implemented with local persistence and UI smoke coverage | `CheckInSheet`, `AppState.recordCheckIn`, snapshot store, `LogEntry.loggedAt`, typed check-in payloads, `testTodayCheckInSavesToTimelineSmoke`; timeline editing can update local title/details and deletion can remove entries after confirmation with a local undo toast | Add backend sync and structured type-specific edit fields |
| Bowel movement logging | Implemented with local persistence and UI smoke coverage | Quick Bristol and detailed bowel form call `recordBowel`; typed bowel payloads persist Bristol, urgency, blood, mucus, pain, and nighttime fields; `testBowelLogWithSignificantBloodShowsSafetyGuidanceSmoke`; timeline editing can update local title/details and deletion can remove entries after confirmation with a local undo toast | Add backend sync, type-specific edit fields, and stricter validation |
| Food/meal logging | Implemented with local persistence and UI smoke coverage | Quick food and Food form insert timeline logs with typed food payloads; `testFoodLogSavesPatternEntrySmoke` verifies pattern-tracking wording; `testInsightsPopulatedSummaryUsesLocalLogsSmoke` verifies food-frequency appearance in populated Insights; timeline editing can update local title/details and deletion can remove entries after confirmation with a local undo toast | Add backend sync, recent/favorite foods, structured edit fields, and broader populated food-pattern Insights scenarios |
| Medication tracking | Implemented with local persistence and UI smoke coverage | Quick meds and full-row Meds form dose buttons update dose count/logs with typed medication payloads; `testMedicationDoseUpdatesHomeSummarySmoke` | Add persistent schedules, skipped/missed-dose states, reminders, and backend sync |
| Symptom logging | Implemented with local persistence | Sliders save symptom timeline entry with typed pain/fatigue/mood payload | Add notes, red-flag linkage, backend sync, and type-specific edit UI |
| Sleep logging | Implemented with local persistence | Sleep form saves quality/wake entry with typed sleep payload | Add editable times/duration and backend sync |
| Weight logging | Implemented with local persistence | Weight form saves manual entry with typed value/unit payload | Add units/validation and backend sync |
...
| Timeline log edit/delete/undo | Implemented locally with edit sheet, destructive confirmation, undo toast, and UI smoke coverage | `AppState.updateLog`, `AppState.deleteLog`, `AppState.performToastAction`, payload clearing on generic text edit, `healthLogUpdate`, `healthLogDeletion`, `testUpdateLogPersistsAndCoalescesPendingCreate`, `testDeleteLogRemovesEntryAndCoalescesPendingCreate`, `testUndoDeleteRestoresLogAndPendingMutations`, `testTimelineEntryEditUpdatesLocalRowSmoke`, `testTimelineEntryDeleteRequiresConfirmationSmoke`, `testTimelineEntryDeleteUndoRestoresLocalRowSmoke` | Add backend update/delete replay, conflict handling, receipts, and type-specific edit forms that update payloads directly |
| Offline sync queue | Implemented locally with replay planning and UI smoke coverage | `PendingSyncMutation`, `SyncReplayPlanItem`, `LocalSyncReplayWorker`, idempotency/server/receipt metadata, `healthLogUpdate`, `healthLogDeletion`, Profile sync row, queue persistence/edit/delete/replay-plan/legacy-mutation tests, `testProfileSyncRetryShowsBackendBlockedSmoke` | Connect Supabase replay client, returned server IDs/receipts, conflicts, backoff, reachability, and user-visible per-record detail |
...
| Insights | Implemented locally with empty, populated, typed-payload, date-windowed, and chart-summary coverage | Local logs with `loggedAt` timestamps and typed payloads drive 7-day Recent and All summaries, bowel chart, pain heatmap, food frequency rows, empty states, and populated chart accessibility labels; `testInsightSummaryPrefersTypedPayloadOverDisplayText`; `testInsightSummaryRecentRangeUsesLoggedAtDates`; `testInsightsEmptyStateAvoidsDemoClaimsSmoke`; `testInsightsPopulatedSummaryUsesLocalLogsSmoke` | Add custom range UI, backend-synced summaries, richer populated/partial scenarios, export/share integration, type-specific edit payload preservation, and manual VoiceOver/Dynamic Type verification |
...
- Tests: passing with 35 unit tests through `InflamendTests` and 21 UI smoke tests through `InflamendUITests`.
- Improvement passes completed: pass 1 is complete; pass 2 is in progress.
- Core flows: auth sign-up/sign-in/onboarding with UI smoke coverage, Profile sign-out with UI smoke coverage, Profile sync status with replay planning/idempotency metadata and blocked-retry UI smoke coverage, Profile privacy toggles with UI smoke coverage, Today check-in with UI smoke coverage, structured log timestamps with legacy decode coverage, typed local log payloads with legacy fallback, date-windowed Insights/report summaries, timeline log edit/delete/undo with UI smoke coverage, bowel red-flag logging with UI smoke coverage, food logging with UI smoke coverage, medication dose tracking with UI smoke coverage, voice transcript confirmation with editable-field UI smoke coverage, voice permission fallback with UI smoke coverage, session restore, local persistence, pending sync queue, logging, local-log Insights with empty, populated, typed-payload, and chart accessibility-summary UI smoke coverage, local text doctor-report export with Profile UI smoke coverage, local user-data JSON export with Profile UI smoke coverage, local Care safety responses with red-flag and medication-refusal UI smoke coverage, destructive-action confirmations with UI smoke coverage, safety, privacy, and report scaffolds improved; live Supabase auth/sync/backend integration remains incomplete.
