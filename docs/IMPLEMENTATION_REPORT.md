...
What remains:
- Add user-managed medication schedule creation/editing, skipped/missed-dose history, reminder scheduling, backend medication sync, and manual Dynamic Type/VoiceOver verification for the richer status row.
...
| Medication tracking | Implemented with persisted local dose status, structured timeline editing, and UI smoke coverage | `MedicationDoseStatus`, snapshot-backed `medicationDoses`, quick meds and full-row Meds form dose buttons update dose count/logs with typed medication payloads; Home timeline editing can update medication dose status, replay snapshots, and adherence summary state; `testMedicationDoseStatusPersistsAndQueuesStructuredLog`; `testMedicationDoseUpdatesHomeSummarySmoke`; `testMedicationLogCanBeEditedFromTimelineSmoke`; `testUpdateLogCanReplaceMedicationPayloadAndReconcileAdherence` | Add user-managed schedule creation/editing, reminder scheduling, backend sync, skipped/missed-dose history views, and manual accessibility verification for medication edit controls |
